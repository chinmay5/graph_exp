...
from train_GCN import run_training
...
def train_fold(train_ind, test_ind, val_ind, graph_feat, features, y, y_data, params, subject_IDs):
    ...
    y = y - 1  # Bring within range of [0, 1]
    test_acc, test_auc = run_training(adj=final_graph, features=x_data, labels=y, idx_train=train_ind, idx_val=val_ind,
                                      idx_test=test_ind, params=params)

    print(test_acc)

    # return number of correctly classified samples instead of percentage
    test_acc = int(round(test_acc.item() * len(test_ind)))
...
def main():
    ...
    params['max_degree'] = 4  # Maximum Chebyshev polynomial degree.
    params[
    ...
    if args.folds == 11:  # run cross validation on all folds
        ...
        for train_ind, test_ind in reversed(list(skf.split(np.zeros(num_nodes), np.squeeze(y)))):
            test_acc, test_auc, lin_acc, lin_auc, fold_size = train_fold(train_ind, test_ind, test_ind, graph, features,
                                                                         y, y_data, params, subject_IDs)
            scores_acc.append(test_acc)
            scores_auc.append(test_auc)
            scores_lin.append(lin_acc)
            scores_auc_lin.append(lin_auc)
        ...
        print(f'overall linear accuracy {str(np.sum(scores_lin) * 1. / num_nodes)}')
        print(f'overall linear AUC {str(np.mean(scores_auc_lin))}')
        print(f'overall accuracy {str(np.sum(scores_acc) * 1. / num_nodes)}')
        print(f'overall AUC {str(np.mean(scores_auc))}')
